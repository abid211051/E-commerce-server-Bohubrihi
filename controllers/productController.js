...
module.exports.createProduct = async (req, res) => {
  try {
    ...
    const imageurl = `${process.env.URL}/${req.files[0].filename}`;
...
module.exports.getProductById = async (req, res) => {
  ...
  try {
    ...
    product.photo = `${process.env.URL}/${product.photo}`;
    return res.status(201).send({
...
module.exports.updateProduct = async (req, res) => {
  try {
    ...
    const result = await Product.findOneAndUpdate({ _id: prodId }, updateData, {
      upsert: true,
    });
